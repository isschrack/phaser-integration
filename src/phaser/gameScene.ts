...
export class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Add debug text for ladder status
    this.debugText = this.add
      .text(20, 50, "", { fontSize: "16px", color: "#00ff00" })
      .setDepth(99)
      .setScrollFactor(0);
  ...
  update() {
    ...
    // Update debug text
    this.debugText.setText(`Parts: ${this.collectedItems}/10`);
   ...
   * @param direction 0: left, 1: up (jump), 2: right, 3: diagonal jump left, 4: diagonal jump right
  ...
  handlePlayerMovement(direction?: number, force?: number) {
    ...
    if (typeof direction === "number" && typeof force === "number") {
      ...
      if (direction === 0 || direction === 2) {
        ...
        if (p.body.blocked.down || p.body.blocked.up) {
          p.anims.play("jump");
          p.setFlip(direction === 4, this.isUpDown);
          // Set both horizontal and vertical velocity for diagonal movement
          p.setVelocityX(dx * 3); // Horizontal velocity for diagonal jump
          p.setVelocityY(-Math.abs(force) * 45); // Vertical velocity for diagonal jump
          this.sound.play("jumpAudio");
          this.startJumpVFX();

          // Add a tween to gradually reduce horizontal velocity for more natural movement
          this.tweens.add({
